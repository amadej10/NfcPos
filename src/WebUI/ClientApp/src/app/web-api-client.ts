...
export interface IOperationsClient {
    ...
    withdrawAll(command: WithdrawAllCommand): Observable<BalanceVm>;
}
...
export class OperationsClient implements IOperationsClient {
    ...
    withdrawAll(command: WithdrawAllCommand): Observable<BalanceVm> {
        let url_ = this.baseUrl + "/api/Operations/WithdrawAll";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processWithdrawAll(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processWithdrawAll(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<BalanceVm>;
                }
            } else
    ...
    protected processWithdrawAll(response: HttpResponseBase): Observable<BalanceVm> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export class WithdrawAllCommand implements IWithdrawAllCommand {
    nfcId?: string;

    constructor(data?: IWithdrawAllCommand) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): WithdrawAllCommand {
        data = typeof data === 'object' ? data : {};
        let result = new WithdrawAllCommand();
        result.init(data);
        return result;
    }
...
export interface IWithdrawAllCommand {
    nfcId?: string;
}
